...
- SAFE: never deletes or overwrites body content. Existing folders are
  ...
  first "# ..." heading) is checked/corrected, and broken Markdown links
  are repointed -- everything else is left untouched. Newly adopted
...
--------------------------------------------------------------------------
    ...
    green    [FOLDER CREATED]   \
             [FILE CREATED]      \
             [TITLE FIXED]        > successful, fully automatic actions
             [LINK FIXED]        /
             [FOLDER ADOPTED]    /
             [FILE ADOPTED]     /
...
- New entries are appended after the existing ones when first adopted --
  ...
  re-sorted into strict ascending numeric order (see TREE SORTING
...
def create_markdown_file(path: str) -> None:
    ...
    corrected if needed (see ensure_title_format).
    """
...
def adopt_file(node: dict, folder_path: str, filename: str) -> None:
    ...
    on disk if needed, checks/fixes its title line, and appends it to
    node["files"].
    """
...
def is_placeholder_only(content: str) -> bool:
    """
    Returns True if `content` is nothing but the title line, optionally
    followed by the default "Documentation pending." placeholder and/or
    blank lines.
    """
...
def load_structure(structure_file: str) -> dict:
    ...
    if not os.path.isfile(structure_file):
        print(f"{tag('ERROR')}Could not find '{structure_file}'. "
              f"Make sure it sits next to documentation-manager.py.", file=sys.stderr)
        sys.exit(1)
