# 03 — Roadmap

This roadmap tracks Project Elysium's development phases at a high level. Detailed completed work is logged per-version in [004-Version-History.md](004-Version-History.md); long-term ideas beyond this roadmap live in [005-Future-Plans.md](005-Future-Plans.md).

## Phase 0 — Pre-Production *(current)*
* Finalize core lore, races, classes, and world structure (this documentation set).
* Concept art and art style guide ([130-Art-Style.md](130-Art-Style.md)).
* Prototype combat and stat formulas ([044-Combat.md](044-Combat.md), [045-Stats.md](045-Stats.md)).
* Stand up plugin architecture skeleton ([120-Plugin-Architecture.md](120-Plugin-Architecture.md)).

## Phase 1 — Core Engine Development
* Server plugin framework, database schema, authentication ([121-Database.md](121-Database.md), [124-Authentication.md](124-Authentication.md)).
* Launcher MVP ([110-Launcher.md](110-Launcher.md)).
* Core UI framework and HUD.

## Phase 2 — Gameplay Systems
* Classes, talent trees, skills, itemization ([040-Classes.md](040-Classes.md) through [059-Enchanting.md](059-Enchanting.md)).
* Quest and dialogue systems.
* Economy foundations ([100-Economy.md](100-Economy.md)).

## Phase 3 — World Building
* Continent 1 (Aurelia) and Continent 2 (Vethmoor) construction.
* First main story arc, first two dungeons, first raid wing.

## Phase 4 — Closed Alpha
* Internal and invited testers. Core loop validation, combat balance passes.

## Phase 5 — Closed Beta
* Wider testing, load testing, launcher and account systems hardened.

## Phase 6 — Public Release
* Launch with Aurelia + Vethmoor, factions live, first raid tier, launcher public.

## Phase 7 — Live Service
* Seasonal events, balance patches, quality-of-life updates.

## Phase 8 — Future Expansions
* Additional continents and systems per [150-Expansion-01.md](150-Expansion-01.md) onward.
